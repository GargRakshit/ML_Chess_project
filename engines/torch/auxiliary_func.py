...
def get_num_classes_from_mapping(mapping_path):
    """ Loads a pickled mapping file and returns its length. """
    try:
        with open(mapping_path, "rb") as f: move_to_int = pickle.load(f)
        return len(move_to_int)
    except Exception as e: print(f"Error loading map {mapping_path}: {e}"); return None